...
export default function Login() {
  const signup = location.pathname === '/inscription' || location.pathname === '/demo'
  ...
  return (
    <div className="login-glass flex min-h-screen items-center justify-center p-4">
      <form onSubmit={submit} className="card w-full max-w-sm p-6">
        ...
        {storage === 'none' && <p className="mb-3 rounded-lg bg-amber-50 p-3 text-xs text-amber-900">L’espace sécurisé est en cours d’activation (stockage non connecté). Réessayez plus tard.</p>}
        {signup && <><label className="label" htmlFor="signup-name">{t("Votre nom", "Your name")}</label><input id="signup-name" className="input mb-3" required maxLength={120} autoComplete="name" value={name} onChange={e => setName(e.target.value)} /><label className="label" htmlFor="signup-agency">{t("Nom de l’agence", "Agency name")}</label><input id="signup-agency" className="input mb-3" required maxLength={160} autoComplete="organization" value={agencyName} onChange={e => setAgencyName(e.target.value)} /><p className="mb-3 text-sm">{t("Essai gratuit de 3 jours, puis abonnement requis. Votre espace privé accueille 3 membres pour démarrer. Aucun compte externe requis pour utiliser les outils internes.", "Free 3-day trial, then a subscription is required. Your private workspace includes 3 team members to get started. No external account needed for internal tools.")}</p></>}
        ...
        <div className="mt-4 flex justify-between text-xs text-slate-500">
          <a href="/" className="hover:underline">{t("Accueil", "Home")}</a>
