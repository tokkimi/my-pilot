    ...
    <Modal title={m.name || 'Nouveau membre'} onClose={onClose}
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        ...
        <div className="rounded-lg border border-slate-200 p-3 sm:col-span-2">
          <div className="mb-2 text-sm font-semibold">Autorisations des connexions personnelles</div>
          <p className="mb-3 text-xs text-slate-500">Pour chaque plateforme : aucun accès, consultation d’un compte déjà connecté, ou gestion de la connexion. Ces droits ne partagent pas automatiquement le compte d’un autre membre et n’activent pas les intégrations indisponibles.</p>
          {(['meta', 'calendly', 'canva', 'linkedin', 'microsoft'] as const).map(key => <label key={key} className="mb-2 flex items-center justify-between gap-3 text-sm"><span>{({meta:'Facebook / Instagram',calendly:'Calendly',canva:'Canva',linkedin:'LinkedIn',microsoft:'Microsoft 365'})[key]}</span><select className="input w-auto" disabled={m.role === 'admin'} value={m.role === 'admin' ? 'manage' : m.toolAccess?.[key] ?? 'none'} onChange={e => set('toolAccess', {...m.toolAccess,[key]:e.target.value as 'none'|'read'|'manage'})}><option value="none">Aucun accès</option><option value="read">Consultation</option><option value="manage">Gérer la connexion</option></select></label>)}
          <div className="mb-1 mt-4 text-sm font-semibold">Accès aux outils Google</div>
