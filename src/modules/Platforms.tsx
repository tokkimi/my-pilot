...
export default function Platforms(_: PageProps) {
  ...
  return (
    <div>
      <PageHeader title={tr("Plateformes")} subtitle={tr("Tous les outils de l’agence au même endroit — un clic pour ouvrir")}
      ...
      <div className="relative mb-4">
        ...
        <input className="input pl-9" placeholder={tr("Rechercher une plateforme…")} value={q} onChange={e => setQ(e.target.value)} />
      </div>
      {cats.map(cat => (
        <section key={cat} className="mb-6">
          <h2 className="mb-4 text-xl font-semibold text-slate-800">{cat}</h2>
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {list.filter(p => p.category === cat).map(p => (
              <div key={p.id} className="card platform-tile group flex items-start gap-4 p-5">
                <div className="platform-icon flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl">
                  {p.url ? <img src={favicon(p.url)} alt="" className="h-8 w-8 object-contain" loading="lazy" onError={e => { e.currentTarget.style.display = 'none' }} /> : <span>🔗</span>}
                </div>
                <div className="min-w-0 flex-1">
                  ...
                  {guideFor(p) && <button className="mt-1 flex items-center gap-1 text-xs font-medium text-brand-700 hover:underline" onClick={() => setGuide({ g: guideFor(p)!, p })}><BookOpen size={12} />{" "}{tr("Guide d’utilisation")}</button>}
                  {p.account && <div className="mt-1 truncate text-xs"><span className="text-slate-400">Compte :</span> {p.account}</div>}
                </div>
                <button className="btn-ghost p-1" onClick={() => setEditing(p)}><Pencil size={14} /></button>
              </div>
            ))}
          </div>
        </section>
  ...
  return (
    <Modal title={`Guide — ${g.name}`} onClose={onClose} footer={<>
      ...
      <div className="space-y-4 text-sm">
        ...
        <div><h3 className="mb-1 font-semibold">{tr("C’est quoi?")}</h3><p className="text-slate-700">{g.what}</p></div>
        <div><h3 className="mb-1 font-semibold">{tr("Quand l’utiliser dans nos processus")}</h3><ul className="list-disc space-y-1 pl-5 text-slate-700">{g.when.map(x => <li key={x}>{x}</li>)}</ul></div>
        <div><h3 className="mb-1 font-semibold">{tr("Comment faire")}</h3><ol className="list-decimal space-y-1 pl-5 text-slate-700">{g.how.map(x => <li key={x}>{x}</li>)}</ol></div>
        {g.tips && <div className="rounded-lg bg-amber-50 p-3 text-amber-900">💡 {g.tips.join(' ')}</div>}
        <div className="rounded-lg bg-brand-50 p-3 text-brand-700"><b>{tr("Avec ImmoPilot :")}</b> {g.integration.text}</div>
        {p.account && <p className="text-slate-600">{tr("Compte utilisé :")}{" "}<b>{p.account}</b></p>}
...
function PlatformForm({ p: init, onClose }: { p: Platform; onClose: () => void }) {
  ...
  return (
    <Modal title={exists ? p.name : 'Nouvelle plateforme'} onClose={onClose}
      footer={<>
        {exists && <button className="btn-ghost mr-auto text-rose-600" onClick={() => { remove('platforms', p.id); onClose() }}><Trash2 size={15} />{" "}{tr("Retirer")}</button>}
        <button className="btn-ghost" onClick={onClose}>{tr("Annuler")}</button>
        <button className="btn-primary" onClick={() => { if (p.name) { upsert('platforms', p); onClose() } }}>{tr("Enregistrer")}</button>
      </>}>
      <div className="grid gap-3 sm:grid-cols-2">
        <Field label={tr("Nom")}><input className="input" value={p.name} onChange={e => set('name', e.target.value)} /></Field>
        <Field label={tr("Catégorie")}><input className="input" list="pcats" value={p.category} onChange={e => set('category', e.target.value)} />
