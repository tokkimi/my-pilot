...
export default function Partners(_: PageProps) {
  ...
  return (
    <div>
      <PageHeader title="Partenaires" subtitle="Notaires, arpenteurs, inspecteurs, courtiers hypothécaires, photographes, home staging…"
        actions={<>
...
function PartnerForm({ p: init, onClose }: { p: Partner; onClose: () => void }) {
  ...
  return (
    <Modal title={exists ? p.name : 'Nouveau partenaire'} onClose={onClose}
      footer={<>
        {exists && <button className="btn-ghost mr-auto text-rose-600" onClick={() => { remove('partners', p.id); onClose() }}><Trash2 size={15} /> Supprimer</button>}
        <button className="btn-ghost" onClick={onClose}>Annuler</button>
        <button className="btn-primary" onClick={() => { if (p.name) { upsert('partners', p); onClose() } }}>Enregistrer</button>
      </>}>
      <div className="grid gap-3 sm:grid-cols-2">
        <Field label="Catégorie"><select className="input" value={p.category} onChange={e => set('category', e.target.value as PartnerCat)}>{Object.entries(PARTNER_CATS).map(([k, v]) => <option key={k} value={k}>{v}</option>)}</select></Field>
        ...
        <Field label="Courriel"><input className="input" value={p.email} onChange={e => set('email', e.target.value)} /></Field>
        <Field label="Site Web"><input className="input" value={p.website} onChange={e => set('website', e.target.value)} /></Field>
