...
export default function Contacts({ openId, go }: PageProps) {
  ...
  return (
    <div>
      <PageHeader title="Contacts & prospects" subtitle={`${list.length} contact(s) — vendeurs, acheteurs, anciens clients, sphère d’influence`}
        actions={<>
          <ScopeFilter />
          ...
          <button className="btn-primary" onClick={() => setEditing(newContact(me.id))}><Plus size={16} /> Nouveau contact</button>
        </>} />
      ...
      <div className="card overflow-x-auto">
        <table className="w-full">
          <thead><tr><th className="th">Nom</th><th className="th">Type</th><th className="th">Étape</th><th className="th hidden md:table-cell">Coordonnées</th><th className="th hidden lg:table-cell">Source</th><th className="th hidden lg:table-cell">Dernier contact</th><th className="th">Resp.</th></tr></thead>
          <tbody>
            {list.map(c => (
  ...
  return (
    <Modal title={contact.firstName ? `Modifier — ${fullName(contact)}` : 'Nouveau contact'} onClose={onClose}
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <Field label="Prénom"><input className="input" value={c.firstName} onChange={e => set('firstName', e.target.value)} autoFocus /></Field>
        <Field label="Nom"><input className="input" value={c.lastName} onChange={e => set('lastName', e.target.value)} /></Field>
        <Field label="Type"><select className="input" value={c.type} onChange={e => set('type', e.target.value as ContactType)}>{Object.entries(CONTACT_TYPES).map(([k, v]) => <option key={k} value={k}>{v}</option>)}</select></Field>
        ...
        <Field label="Courriel"><input className="input" type="email" value={c.email} onChange={e => set('email', e.target.value)} /></Field>
        <Field label="Adresse"><input className="input" value={c.address} onChange={e => set('address', e.target.value)} /></Field>
        <Field label="Ville"><input className="input" value={c.city} onChange={e => set('city', e.target.value)} /></Field>
        ...
        <Field label="Responsable"><MemberSelect value={c.ownerId} onChange={v => set('ownerId', v)} /></Field>
        ...
        <Field label="Notes" className="sm:col-span-2"><textarea className="input min-h-24" value={c.notes} onChange={e => set('notes', e.target.value)} /></Field>
      </div>
    </Modal>
  )
}
