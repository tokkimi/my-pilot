...
export default function Marketing(props: PageProps) {
  ...
  return <div className="marketing-studio">
    <PageHeader title={tr("Studio marketing")} subtitle={`${db.agency.name} · ${tr('Préparez et coordonnez votre communication')}`}
      actions={<button className="btn-primary" onClick={() => collection ? setEditing(fresh(collection, me.id)) : setPost(newPost({ ownerId: me.id }))}><Plus size={17} /> {collection ? tr(TITLES[collection]) : tr('Publication')}</button>} />
    <nav aria-label={tr("Studio marketing")} className="mb-6 flex flex-wrap gap-2">{SECTIONS.map(([k, label]) => <button key={k} aria-current={section === k ? 'page' : undefined} className={section === k ? 'btn-primary' : 'btn-outline'} onClick={() => { setSection(k); setQuery(''); setOwner('') }}>{typeof label === 'string' ? tr(label) : label}</button>)}</nav>
    {section === 'overview' && <>
      ...
      ].map(([label, value]) => <div key={typeof label === 'string' ? tr(label) : label} className="card p-5"><p className="text-sm text-slate-500">{typeof label === 'string' ? tr(label) : label}</p><p className="mt-2 text-3xl font-semibold">{value}</p></div>)}</div>
      <div className="grid gap-5 lg:grid-cols-2"><section className="card p-5"><h2 className="mb-4 text-xl font-semibold">{tr("Prochaines publications")}</h2>{upcoming.length ? upcoming.map(p => <button key={p.id} className="block w-full border-b border-slate-100 py-3 text-left" onClick={() => setPost(p)}><span className="text-sm text-slate-500">{p.date} {p.time} · {p.platforms.join(', ')}</span><p className="line-clamp-2">{p.caption || p.kind}</p></button>) : <Empty>{tr("Aucune publication à venir. Préparez votre premier contenu.")}</Empty>}</section>
      <section className="card p-5"><h2 className="mb-4 text-xl font-semibold">{tr("Validation de l’équipe")}</h2>{pending.length ? pending.map(p => <button key={p.id} className="block w-full border-b border-slate-100 py-3 text-left" onClick={() => setPost(p)}>{p.caption || p.kind}<p className="text-sm text-slate-500">{db.members.find(m => m.id === p.ownerId)?.name || 'Sans responsable'}</p></button>) : <Empty>{tr("Aucun contenu en attente de validation.")}</Empty>}</section></div>
    ...
    {collection && <>
      {section === 'account' && <section className="card mb-5 p-5"><h2 className="text-xl font-semibold">{tr("Vos comptes et outils de publication")}</h2><p className="my-3 text-slate-600">{tr("Référencez les comptes de l’agence et leur responsable. La connexion OAuth, la publication automatique et la synchronisation des statistiques ne sont pas encore activées. Les liens ouvrent les outils officiels.")}</p><div className="flex flex-wrap gap-2">{PROVIDERS.map(([name, url]) => <a key={name} className="btn-outline" href={url} target="_blank" rel="noreferrer"><ExternalLink size={16} />{name}</a>)}</div></section>}
      {section === 'campaign' && <p className="mb-4 text-slate-600">{tr("Préparez vos briefs, audiences, budgets et créations. Les résultats sont saisis par l’équipe ; aucune dépense publicitaire n’est engagée ici.")}</p>}
      <div className="mb-4 flex flex-wrap gap-3"><input aria-label="Rechercher dans le studio" className="input max-w-md" placeholder={tr("Rechercher…")} value={query} onChange={e => setQuery(e.target.value)} /><select className="input max-w-xs" aria-label="Filtrer par responsable" value={owner} onChange={e => setOwner(e.target.value)}><option value="">{tr("Tous les responsables")}</option>{db.members.map(m => <option key={m.id} value={m.id}>{m.name}</option>)}</select></div>
      ...
      {!items.some(i => i.kind === collection && matches(i.title, i.ownerId)) && <Empty>Aucun élément{query || owner ? ' ne correspond à ces filtres' : ' pour le moment'}. Utilisez « {tr(TITLES[collection])} » pour commencer.</Empty>}
    </>}
...
function ItemCard({ item: i, onEdit }: { item: MarketingItem; onEdit: () => void }) {
  ...
  return <article className="card overflow-hidden">
    ...
    <div className="p-5"><button onClick={onEdit} className="text-left text-lg font-semibold hover:text-brand-600">{i.title}</button><p className="mt-1 text-sm text-slate-500">{i.kind === 'account' ? 'Référencé · connexion API inactive' : STATUS[i.status]} · {db.members.find(m => m.id === i.ownerId)?.name || 'Sans responsable'}</p>
      ...
      <div className="mt-4 flex flex-wrap gap-2"><button className="btn-outline" onClick={onEdit}>{tr("Ouvrir")}</button>{safeUrl(i.url) && <a className="btn-outline" href={safeUrl(i.url)} target="_blank" rel="noreferrer"><ExternalLink size={15} />{" "}{tr("Lien")}</a>}{mediaUrl && <a className="btn-outline" href={mediaUrl} target="_blank" rel="noreferrer">{tr("Fichier")}</a>}</div>
    </div>
  </article>
}
...
function EditorialCalendar({ posts, events, onPost, onEvent }: { posts: Post[]; events: MarketingItem[]; onPost: (p: Post) => void; onEvent: (i: MarketingItem) => void }) {
  ...
  return <section className="card p-4 sm:p-6"><div className="mb-5 flex flex-wrap items-center justify-between gap-3"><h2 className="text-xl font-semibold capitalize">{month.toLocaleDateString('fr-CA', { month: 'long', year: 'numeric' })}</h2><div className="flex items-center gap-2"><select aria-label="Filtrer le planning par réseau" className="input" value={channel} onChange={e => setChannel(e.target.value)}><option value="">{tr("Tous les réseaux")}</option>{CHANNELS.map(c => <option key={c}>{c}</option>)}</select><button className="btn-outline" aria-label="Mois précédent" onClick={() => setMonth(new Date(month.getFullYear(), month.getMonth() - 1, 1))}><ChevronLeft size={18} /></button><button className="btn-outline" aria-label="Mois suivant" onClick={() => setMonth(new Date(month.getFullYear(), month.getMonth() + 1, 1))}><ChevronRight size={18} /></button></div></div>
    <div className="overflow-x-auto"><div className="grid min-w-[700px] grid-cols-7 gap-1">{['Lundi', 'Mardi', 'Mercredi', 'Jeudi', 'Vendredi', 'Samedi', 'Dimanche'].map(d => <div className="p-2 text-sm text-slate-500" key={d}>{tr(d)}</div>)}{Array.from({ length: offset }, (_, n) => <div key={`blank${n}`} />)}{Array.from({ length: days }, (_, n) => {
    ...
    })}</div></div><p className="mt-4 text-sm text-slate-500">{tr("Cliquez sur un jour pour préparer un contenu. La mise en ligne reste manuelle.")}</p>
  </section>
}
...
function ItemForm({ item, onClose }: { item: MarketingItem; onClose: () => void }) {
  ...
  const text = (key: 'title' | 'objective' | 'audience' | 'url' | 'location' | 'colors' | 'typography' | 'voice' | 'rights' | 'checklist' | 'notes', label: string, multiline = false) => <Field label={typeof label === 'string' ? tr(label) : label}>{multiline ? <textarea className="input min-h-28" value={i[key]} onChange={e => set(key, e.target.value)} /> : <input className="input" type={key === 'url' ? 'url' : 'text'} value={i[key]} onChange={e => set(key, e.target.value)} />}</Field>
  const num = (key: 'budget' | 'spent' | 'impressions' | 'clicks' | 'leads' | 'attendees', label: string) => <Field label={typeof label === 'string' ? tr(label) : label}><input className="input" min="0" type="number" step={key === 'budget' || key === 'spent' ? '0.01' : '1'} value={i[key]} onChange={e => set(key, Math.max(0, Number(e.target.value)))} /></Field>
  ...
  return <Modal title={tr(TITLES[i.kind])} onClose={onClose} wide footer={<>
    {exists && <button className="btn-ghost mr-auto text-rose-700" onClick={() => { if (confirm('Retirer cet élément du studio ? Le fichier source sera conservé.')) { remove('marketingItems', i.id); onClose() } }}><Trash2 size={16} />{" "}{tr("Retirer")}</button>}
    <button className="btn-outline" onClick={onClose}>{tr("Annuler")}</button><button className="btn-primary" disabled={busy || !i.title.trim()} onClick={() => { if (i.url && !safeUrl(i.url)) { setError('Utilisez un lien http ou https valide.'); return } if (i.end && i.end < i.start) { setError('La fin doit être après le début.'); return } upsert('marketingItems', i); onClose() }}>{tr("Enregistrer")}</button>
  </>}><div className="grid gap-4 sm:grid-cols-2">{text('title', i.kind === 'account' ? 'Nom du compte / identifiant public' : 'Nom')}
    <Field label={tr("Responsable")}><MemberSelect value={i.ownerId} onChange={v => set('ownerId', v)} allowEmpty /></Field>
    {i.kind !== 'account' && <Field label={tr("Statut")}><select className="input" value={i.status} onChange={e => set('status', e.target.value as MarketingItem['status'])}>{Object.entries(STATUS).map(([k, label]) => <option key={k} value={k}>{typeof label === 'string' ? tr(label) : label}</option>)}</select></Field>}
    {['campaign', 'event'].includes(i.kind) && <><Field label={tr("Début")}><input className="input" type="date" value={i.start} onChange={e => set('start', e.target.value)} /></Field><Field label={tr("Fin")}><input className="input" type="date" min={i.start} value={i.end} onChange={e => set('end', e.target.value)} /></Field>{text('objective', 'Objectif', true)}{text('audience', 'Audience / public', true)}{num('budget', 'Budget prévu ($ CA)')}{num('spent', 'Dépenses engagées ($ CA)')}{text('checklist', 'Plan d’action et checklist', true)}</>}
    ...
    {i.kind === 'asset' && <>{text('rights', 'Crédit, licence et autorisations', true)}<Field label={tr("Expiration des droits")}><input className="input" type="date" value={i.expires} onChange={e => set('expires', e.target.value)} /></Field></>}
    {['asset', 'brand'].includes(i.kind) && <Field label={tr("Fichier / logo")}><div className="rounded-xl border border-dashed p-4"><Upload size={20} className="mb-2" /><input aria-label="Ajouter un fichier au studio" type="file" disabled={busy} accept="image/jpeg,image/png,image/webp,image/gif,video/mp4,video/webm,application/pdf" onChange={e => void upload(e.target.files?.[0])} /><p className="mt-2 text-sm">{busy ? 'Téléversement…' : i.media?.name || 'Photos, vidéos, logos ou PDF'}</p></div></Field>}
