...
export default function Marketing(_: PageProps) {
  ...
  return (
    <div>
      <PageHeader title="Publications" subtitle="Préparation, validation et suivi des publications manuelles"
  ...
  return (
    <Modal title={exists ? 'Modifier la publication' : 'Nouvelle publication'} onClose={onClose}
      footer={<>
        ...
        <button className="btn-primary" disabled={!p.caption.trim() || !p.date || (p.status === 'planifie' && p.approval !== 'approuve')} onClick={() => { upsert('posts', p); onClose() }}>Enregistrer</button>
      </>}>
      <div className="grid gap-3 sm:grid-cols-2">
        <Field label="Responsable"><MemberSelect allowEmpty value={p.ownerId ?? ''} onChange={v => set('ownerId', v)} /></Field>
        <Field label="Heure (heure locale de l’équipe)"><input className="input" type="time" value={p.time ?? ''} onChange={e => set('time', e.target.value)} /></Field>
        <Field label="Validation"><select className="input" value={p.approval ?? 'brouillon'} onChange={e => set('approval', e.target.value as Post['approval'])}><option value="brouillon">Brouillon</option><option value="a_valider">À valider</option><option value="approuve" disabled={!canApprove}>Approuvé</option></select></Field>
        <Field label="Campagne"><select className="input" value={p.campaignId ?? ''} onChange={e => set('campaignId', e.target.value)}><option value="">Aucune</option>{(db.marketingItems ?? []).filter(i => i.kind === 'campaign').map(i => <option key={i.id} value={i.id}>{i.title}</option>)}</select></Field>
        ...
        <fieldset className="sm:col-span-2"><legend className="label">Visuels de la médiathèque</legend><div className="flex flex-wrap gap-3">{(db.marketingItems ?? []).filter(i => i.kind === 'asset').map(i => <label key={i.id} className="text-sm"><input type="checkbox" checked={(p.assetIds ?? []).includes(i.id)} onChange={e => set('assetIds', e.target.checked ? [...(p.assetIds ?? []), i.id] : (p.assetIds ?? []).filter(id => id !== i.id))} /> {i.title}</label>)}</div></fieldset>
        <Field label="Notes et retours de l’équipe" className="sm:col-span-2"><textarea className="input" value={p.notes ?? ''} onChange={e => set('notes', e.target.value)} /></Field>
        <Field label="Lien de la publication publiée" className="sm:col-span-2"><input className="input" type="url" value={p.publishedUrl ?? ''} onChange={e => set('publishedUrl', e.target.value)} /></Field>
      ...
      <p className="mt-3 text-sm text-slate-500">La planification organise le travail de l’équipe. Elle ne déclenche aucune publication automatique. Validez le contenu avant de le planifier ; toute modification du texte, des visuels ou de la date demande une nouvelle validation.</p>
    </Modal>
  )
}
